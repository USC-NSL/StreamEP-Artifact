...
cleanup(){ iface_stop 2>/dev/null || true; kill_server; "$RAY_BIN" stop --force >/dev/null 2>&1 || true; }
...
launch_server(){
  ...
  )   # NOTE: intentionally NO --gate-profile-file  => gating disabled (minor effect on this metric)
...
log "StreamInfer interference sweep | model=$MODEL layers=$NUM_LAYERS world=$WORLD_SIZE rate=$RATE time=${BENCH_TIME}s conditions=[$CONDITIONS]"
start_ray_head
wait_for_gpus || exit 1
server_log="$RESULTS_DIR/server.log"
log "Launching StreamInfer server (once, reused for all conditions)..."
launch_server "$server_log"
wait_for_server "$server_log" || { tail -30 "$server_log"; exit 1; }
